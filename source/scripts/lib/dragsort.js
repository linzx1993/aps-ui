...
//通过全局的isMobile变量判断是否为移动端，如果是移动端，则调用zepto的方法
let down = isMobile ? "touchstart" : "mousedown",
    move = isMobile ? "touchmove" : "mousemove",
    up = isMobile ? "touchend" : "mouseup",
    doc = isMobile ? z(document) : $(document);


doc
	.on(down,function (e) {
		if(!isMobile && $(".jResultPage").length !== 0 && $(".diffe-table").length === 0){
			e.clientX = isMobile ?  e.targetTouches[0].clientX : e.clientX;
			e.clientY = isMobile ? e.targetTouches[0].clientY : e.clientY;
			isMouseUp = true;
			isDown = true ;
	...
	.on(move,function (e) {
		if(!isMobile && $(".jResultPage").length !== 0 && $(".diffe-table").length === 0){
			(function(){if(!isDown) return isDown ;
				e.clientX = isMobile ? e.targetTouches[0].clientX : e.clientX;
				e.clientY = isMobile ? e.targetTouches[0].clientY : e.clientY;
