...
<script>
import Clickoutside from '@/scripts/utils/clickoutsider';
import Emitter from '@/scripts/mixins/emitter';
...
export default{
	...
	watch: {
		...
		value(val){
			...
			//更新全选状态
			if(this.value && (typeof(this.value) === 'number' || this.value.length)){
				this.checkedAll = true;
				this.broadcast('apsOption', 'queryChange', this.query.toLowerCase());
			}else{
