	 ...
	 **/
	  ...
	  **/
		$scope.around = function () {
			...
			$http.post($rootScope.restful_api.aps_rate_ar_button, JSON.parse(sessionStorage.getItem("papBodyData"))).success(function () {
				$scope.newprogressBar_post("#progressbar_one", "#progress-label_one", $rootScope.restful_api.aps_rate_around, JSON.parse(sessionStorage.getItem("papBodyData")), "前拉后推", function () {}, function (res) {
					$timeout(function () {
						var stateList = res.stateList;
						if (res.rate == 100 && stateList.length == 0) {
							/*9.19 modify by zh*/
							stateList = undefined;
	  ...
	  **/
		$scope.differ_show = function (workDate, equipmentId) {
			...
			$http.get(sUrl).success(function (res) {
				...
				leftList = taskLeft, rightList = taskRight, cpList = res.compareList;
	  ...
	  */
		$scope.askRequest = function (_obj) {
			...
			$http[_obj._type](_obj._url, _data).then(function (res) {
				//刷新二级页面和暂存区
				$scope.cache_box();
